...
module.exports = class ExtraTorrentAPI {
  ...
  constructor(options, debug) {
    ...
    this._request = req.defaults(options || defaultOptions);
  ...
  _get(uri, qs, retry) {
    ...
    var p = new Promise(function(resolve, reject) {
      ...
      _this._request(reqOpts, function(err, res, body) {
        if (err && retry) {
  ...
  _formatPage(res, page, date) {
    ...
    $("tr.tlr , tr.tlz").each(function() {
      ...
      result.results.push({ torrent_link:torrent_link, language:language, title:title, sub_category:sub_category, comments:comments, size:size, seeds:seeds, leechers:leechers, peers:peers, quality:quality });
    });
  ...
  _advancedSearch(opts, date) {
    ...
    var s_cat = this._s_cat[category] || "";
    
    var _this = this;
    return this._get("/advanced_search/", { page:page, "with": with_words, extact:extact, without:without, "s_cat":s_cat, added:added, seeds_from:seeds_from, seeds_to:seeds_to, leechers_from:leechers_from, leechers_to:leechers_to, size_from:size_from, size_to:size_to, size_type:size_type })
      .then(function(res) {
          return _this._formatPage(res, page, Date.now() - date);
      });
  };
